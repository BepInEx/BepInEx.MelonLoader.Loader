...
namespace MelonLoader
{
	...
	{
		private static string ConfigFileName = "MelonLoader.cfg";
		private static IniFile ConfigFile = null;

		private static Dictionary<string, Dictionary<string, MelonPreference>> prefs =
			new Dictionary<string, Dictionary<string, MelonPreference>>();
		...
		{
			if (ConfigFile == null)
				ConfigFile = new IniFile(Path.Combine(BepInEx.Paths.ConfigPath, ConfigFileName));
		}
		...
		public static void RegisterString(string section, string name, string defaultValue, string displayText = null,
			bool hideFromList = false)
		{
			Register(section, name, defaultValue, displayText, MelonPreferenceType.STRING, hideFromList);
		}

		public static void RegisterBool(string section, string name, bool defaultValue, string displayText = null,
			bool hideFromList = false)
		{
			Register(section, name, defaultValue ? "true" : "false", displayText, MelonPreferenceType.BOOL,
				hideFromList);
		}

		public static void RegisterInt(string section, string name, int defaultValue, string displayText = null,
			bool hideFromList = false)
		{
			Register(section, name, "" + defaultValue, displayText, MelonPreferenceType.INT, hideFromList);
		}

		public static void RegisterFloat(string section, string name, float defaultValue, string displayText = null,
			bool hideFromList = false)
		{
			Register(section, name, "" + defaultValue, displayText, MelonPreferenceType.FLOAT, hideFromList);
		}

		private static void Register(string section, string name, string defaultValue, string displayText,
			MelonPreferenceType type, bool hideFromList)
		{
			if (prefs.TryGetValue(section, out Dictionary<string, MelonPreference> prefsInSection))
			{
				...
				else
				{
					...
					prefsInSection.Add(name,
						new MelonPreference(toStoreValue, type, hideFromList,
		...
		{
			return prefs.TryGetValue(section, out Dictionary<string, MelonPreference> prefsInSection) &&
			       prefsInSection.ContainsKey(name);
		}
		...
		{
			if (prefs.TryGetValue(section, out Dictionary<string, MelonPreference> prefsInSection) &&
			    prefsInSection.TryGetValue(name, out MelonPreference pref))
				return pref.Value;
		...
		{
			SetString(section, name, value ? "true" : "false");
		}
