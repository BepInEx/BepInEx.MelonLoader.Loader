...
namespace MelonLoader.Il2CppAssemblyGenerator
{
    internal class PackageBase
    {
        ...
        internal virtual bool Download(bool directory_check)
        {
            ...
            try
            {
                using (var zip = new ZipFile(zipStream, true))
                {
                    foreach (ZipEntry zipArchiveEntry in zip)
                    {
                        if (!zipArchiveEntry.IsFile)
                            continue;

                        MelonLogger.Msg($"Extracting {zipArchiveEntry.Name}");
                        using var dataStream = zip.GetInputStream(zipArchiveEntry);
                        using var fileStream = File.OpenWrite(Path.Combine(Destination, zipArchiveEntry.Name));

                        dataStream.CopyTo(fileStream);
                    }

                    MelonLogger.Msg($"Extracted all");
                }

                MelonLogger.Msg($"Finished extracting");
            }
            catch (Exception ex)
            {
                MelonLogger.Error(ex.ToString());
